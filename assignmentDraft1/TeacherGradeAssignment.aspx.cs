...
{
    public partial class TeacherGradeAssignment : System.Web.UI.Page
    {
        ...
        private void LoadLecturerInfo()
        {
            ...
            using (SqlConnection con = new SqlConnection(cs))
            {
                ...
                // Verify that this assignment belongs to one of this lecturer's modules
                SqlCommand verifyCmd = new SqlCommand(@"
                    SELECT COUNT(*) FROM Assignments A
                    JOIN Modules M ON A.ModuleID = M.ModuleID
                    WHERE A.AssignmentID = @AssignmentID AND M.LecturerID = @LecturerID", con);
                verifyCmd.Parameters.AddWithValue("@AssignmentID", assignmentId);
                verifyCmd.Parameters.AddWithValue("@LecturerID", lecturerId);

                int matchCount = (int)verifyCmd.ExecuteScalar();
                if (matchCount == 0)
                {
                    Response.Redirect("assignments.aspx"); // Not authorized to view this assignment
                }
            }
        }
        ...
        private void SaveGrade(int userId, RepeaterItem item)
        {
            try
            {
                ...
                TextBox txtPoints = (TextBox)item.FindControl("txtPoints");
                ...
                // Validate points are between 0-100
                if (pointsEarned < 0 || pointsEarned > 100)
                {
                    ShowMessage("Points must be between 0 and 100.", "red");
                    return;
                }
                ...
                using (SqlConnection con = new SqlConnection(cs))
                {
                    ...
                    // Use the lecturerId that we already have from LoadLecturerInfo()
                    int gradedByLecturerId = lecturerId;
        ...
        WHERE AssignmentID = @AssignmentID AND UserID = @UserID", con);
            ...
            VALUES (@SubmissionID, @AssignmentID, @UserID, @SubmissionText, @SubmissionDate, @Status, @IsLate)", con);
                        ...
                        createSubmissionCmd.Parameters.AddWithValue("@SubmissionText", "Graded by lecturer without submission");
            ...
            VALUES (@GradeID, @SubmissionID, @GradedBy, @PointsEarned, @Feedback, @GradedDate)", con);
                        ...
                        insertGradeCmd.Parameters.AddWithValue("@GradedBy", gradedByLecturerId); // Fixed: Use LecturerID instead of UserID
            ...
            WHERE SubmissionID = @SubmissionID", con);
                        ...
                        updateGradeCmd.Parameters.AddWithValue("@GradedBy", gradedByLecturerId); // Fixed: Use LecturerID instead of UserID
        ...
        private void SendBulkReminder()
        {
            ...
            ShowMessage("Reminder emails sent to students who haven't submitted yet.", "blue");
        }

        // Helper methods for displaying submission content
        // Make these methods static to ensure they can be called from ASPX
        public static string GetSubmissionContent(object dataItem)
        {
            ...
            content += $"<div class='detail-value' style='color: {(isLate ? "var(--red)" : "var(--green)")}'>{(isLate ? "Late" : "On Time")}</div>";
            content += "</div>";
            content += "</div>";
        ...
        // Make these methods static to ensure they can be called from ASPX
        public static string GetNoSubmissionContent()
        {
            return "<div class='no-submission'>" +
                   "<i class='fas fa-exclamation-triangle' style='font-size: 2rem; margin-bottom: 1rem; color: var(--orange);'></i>" +
                   "<p>Student has not submitted this assignment yet.</p>" +
                   "</div>";
        }

        // Make these methods static to ensure they can be called from ASPX
        public static string GetCurrentGradeDisplay(object pointsEarnedObj, object maxPointsObj, object feedbackObj)
        {
        ...
        // Make this method static as well to be used by GetCurrentGradeDisplay
