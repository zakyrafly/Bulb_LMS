...
{
    public partial class AdminManageLessons : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                ShowMessage("Invalid module ID. Redirecting to course management...", "error");
        ...
        private bool ValidateModuleExists()
        {
            string cs = ConfigurationManager.ConnectionStrings["dbConnection"].ConnectionString;
            using (SqlConnection con = new SqlConnection(cs))
            {
                try
                {
                    ...
                    SqlCommand cmd = new SqlCommand("SELECT COUNT(*) FROM Modules WHERE ModuleID = @ModuleID", con);
                ...
                {
                    ShowMessage("Error validating module existence: " + ex.Message, "error");
                    return false;
                }
            }
        }
            ...
            using (SqlConnection con = new SqlConnection(cs))
            {
                try
                {
                    ...
                    if (reader.Read())
                    {
                        ...
                        lblLecturerName.Text = reader["LecturerName"]?.ToString() ?? "Unassigned";

                        // Store course ID for navigation breadcrumbs if not already set
                        if (courseId == 0)
                        {
                        ...
                        // Update page title
                        Page.Title = $"Admin Manage Lessons - {lblModuleTitle.Text} - Bulb";
                    }
                    else
                    {
        ...
        private void DeleteLesson(int lessonId)
        {
            ...
            using (SqlConnection con = new SqlConnection(cs))
            {
                try
                {
                    ...
                    {
                        ShowMessage("Error deleting lesson. Lesson not found.", "error");
                    }
                }
                catch (Exception ex)
                {
            ...
            using (SqlConnection con = new SqlConnection(cs))
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        {
                            ShowMessage("Error updating lesson. Lesson not found.", "error");
                        }
                    }
        ...
        protected override void OnError(EventArgs e)
        {
            ...
            if (ex != null)
            {
                System.Diagnostics.Debug.WriteLine("AdminManageLessons Page Error: " + ex.Message);
